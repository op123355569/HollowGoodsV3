...
public class EncryptUtils {
    ...
    private static byte[] desCryt(byte[] data, byte[] key, int mode) {
        byte[] result = null;
        try {
    ...
    /**
     * 不足16位时或位数不是16的倍数时，填充数字
     *
     * @param text 内容
     * @param mode 加密/解密
     * @return
     */
    private static List<String> fillNumber(String text, int mode) {
        ...
        is16 = text.length() % 16 == 0 ? true : false;

        for (int i = 0; i < text.length() / 16; i++) {
            result.add(text.substring(i * 16, (i + 1) * 16));
        }

        int addByteCount = 0;
        if (!is16) {
            int lastCount = text.length() % 16;
            addByteCount = 16 - lastCount;
            String temp = text.substring(text.length() / 16 * 16, text.length());
            for (int i = 0; i < addByteCount; i++) {
        ...
        String temp = "";
        if (addByteCount < 10 && mode == MODE_ENCRYPT) {
            temp = "000000000000000" + addByteCount;
            result.add(temp);
        } else if (addByteCount >= 10 && mode == MODE_ENCRYPT) {
            temp = "00000000000000" + addByteCount;
            result.add(temp);
        }
     ...
     * @return
     */
    private static String cutAddBytes(String result) {

        String temp = result.substring(result.length() - 16, result.length());
        int count = Integer.valueOf(temp);
        result = result.substring(0, result.length() - 16 - count);

        return result;
    }

    /**
     * MODE_ENCRYPT 加密模式 MODE_DECRYPT 解密模式
     */
    public static final int MODE_ENCRYPT = 1;
    public static final int MODE_DECRYPT = 2;

    /**
     * DES加密 只能对数字加密最高支持16进制
     *
     * @param hexNumber 小于等于16进制的数字
     * @param key       密钥
     * @param mode      加密/解密
     * @return
     */
    public static String desEncrypt(String hexNumber, String key, int mode) {

        // 加解密模式
        int _mode = Cipher.DECRYPT_MODE;
        if (mode == MODE_ENCRYPT) {
            _mode = Cipher.ENCRYPT_MODE;
        }
        // 被加解密byte数组16进制字符串
        List<String> dataHexString = fillNumber(hexNumber, mode);
        // 秘钥byte数组16进制字符串
        String keyHexString = key;
        String result = "";

        for (int i = 0; i < dataHexString.size(); i++) {

            byte[] data = hexString2Bytes(dataHexString.get(i));
            byte[] _key = hexString2Bytes(keyHexString);
            byte[] _result = desCryt(data, _key, _mode);

            result += bytes2HexString(_result);
        }

        if (mode == MODE_DECRYPT) {
            result = cutAddBytes(result);
        }
        return result;
    }
     ...
     * @return
     */
    private static byte[] threeDesEncryptMode(byte[] keybyte, byte[] src) {
        try {
            // 生成密钥
     ...
     * @return
     */
    private static byte[] threeDesDecryptMode(byte[] keybyte, byte[] src) {
        try {
            // 生成密钥
            SecretKey deskey = new SecretKeySpec(keybyte, Algorithm);
     ...
     * @return
     */
    private static String threeDesByte2String(byte[] byteResult) {

        char[] temp = new char[byteResult.length];
        for (int i = 0; i < byteResult.length; i++) {
            temp[i] = (char) byteResult[i];
        }
     ...
     * @return
     */
    private static byte[] threeDesString2Byte(String strResult) {

        char[] temp = strResult.toCharArray();
     ...
     * @return
     */
    public static String thressDesEncrypt(String text, int mode) {
        ...
        if (mode == MODE_ENCRYPT) {
            return threeDesByte2String(threeDesEncryptMode(keyBytes, text.getBytes()));
        } else if (mode == MODE_DECRYPT) {
            return new String(threeDesDecryptMode(keyBytes, threeDesString2Byte(text)));
        }
     ...
     * @return
     */
    public static String rc4Encrypt(String text, String aKey) {
    ...
    private static final int[] S0, S1, S2, S3; // the s-boxes
    private static final int[] P; // the p-array

    private static boolean encrypting;
    private static byte[] workingKey = null;
     ...
     * @param key
     */
    private static void init(final boolean encrypting, final byte[] key) {
        EncryptUtils.encrypting = encrypting;
        EncryptUtils.workingKey = key;
        setKey(EncryptUtils.workingKey);
    }
     ...
     * @return
     */
    private static byte[] encrypt(final String input) {
        try {
     ...
     * @return
     */
    private static byte[] encrypt(final byte[] source, final int offset, final int length) {
        encrypting = true;
     ...
     * @return
     */
    private static String decryptString(final byte[] source) {
        try {
     ...
     * @return
     */
    private static byte[] decryptBytes(final byte[] source) {
        encrypting = false;
    ...
    private static int processBlock(final byte[] in, final int inOff, final byte[] out, final int outOff) {
        if (workingKey == null) {
     ...
     */
    private static void processTable(int xl, int xr, int[] table) {
     ...
     */
    private static void encryptBlock(byte[] src, int srcIndex, byte[] dst, int dstIndex) {
     ...
     * exact multiple of our blocksize.
     */
    private static void decryptBlock(byte[] src, int srcIndex, byte[] dst, int dstIndex) {
        int xl = BytesTo32bits(src, srcIndex);
        int xr = BytesTo32bits(src, srcIndex + 4);
     ...
     * @return
     */
    public static String blowfishEncrypt(String text, String key, int mode) {

        if (mode == MODE_ENCRYPT) {
            init(true, key.getBytes());
            return threeDesByte2String(encrypt(text));
        } else if (mode == MODE_DECRYPT) {
            init(true, key.getBytes());
            return decryptString(threeDesString2Byte(text));
        }
    ...
    private static String analysisResult(byte[] data) {

        int count = data[data.length - 9];
        byte[] result = new byte[data.length - 8 - count];
        for (int i = 0; i < data.length - 8 - count; i++) {
            result[i] = data[i];
        }
     ...
     * @return
     */
    public static String ideaEncrypt(String text, String key, int mode) {

        if (mode == MODE_ENCRYPT) {
            return threeDesByte2String(IdeaEncrypt(key.getBytes(), text.getBytes(), true));
        } else if (mode == MODE_DECRYPT) {
            return analysisResult(IdeaEncrypt(key.getBytes(), threeDesString2Byte(text), false));
        }
     ...
     *
     * @param text 内容
     * @param key  密钥(固定16个字符,即128位)
     * @param mode 加密/解密
     * @return
     */
    public static String aesEncrypt(String text, String key, int mode) {

        if (mode == MODE_ENCRYPT) {
            try {
                return threeDesByte2String(encrypt(text.getBytes(), key.getBytes()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else if (mode == MODE_DECRYPT) {
            try {
                return new String(decrypt(threeDesString2Byte(text), key.getBytes()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }

...
public class PopupWinHelper {
    ...
    public void init(int layoutRes, int gravity, int animationStyle, String bgColor, int[] id, int[] type, boolean isDeveloping) {
        ...
        if (popupWindow == null || this.layoutRes != layoutRes) {
            ...
            bg.setOnClickListener(new OnViewClickListener(false) {
                @Override
            ...
            if (id != null) {
                for (int i = 0; i < id.length; i++) {
                    ...
                    if (type != null && type.length > i) {
                        switch (type[i]) {
                            case TYPE_ON_CLICK:
                                v.setOnClickListener(new OnViewClickListener(isDeveloping) {
                                    @Override
                                    public void onViewClick(View view, int id) {
                                        if (popupWinOnClickListener != null) {
                                            popupWinOnClickListener.OnPopupWinClick(view);
                                        }
                                    }
                                });
                                break;
                            case TYPE_ON_ITEM_CLICK:
                                ((AbsListView) v).setOnItemClickListener(new OnAdapterViewItemClickListener(isDeveloping) {
                                    @Override
                                    public void onAdapterViewItemClick(AdapterView<?> parentView, View view, int position, long id) {
                                        if (popupWinOnItemClickListener != null) {
                                            popupWinOnItemClickListener.OnPopupWinItemClick(parentView, view, position, id);
                                        }
                                    }
                                });
                                break;
    ...
    public void refreshAbsListViewData(int id, ArrayList<?> data) {
        ...
        BaseAdapter adapter = adapterTags.get(id);
        if (adapter instanceof CommonAdapter) {
            ((CommonAdapter) adapter).refreshData(data);
        } else {
    ...
    public void clearAbsListViewData(int id) {
        ...
        if (adapter instanceof CommonAdapter) {
            ((CommonAdapter) adapter).clearData();
        }
    }

...
public class BGAGlideImageLoader extends BGAImageLoader {
    ...
    @Override
    public void display(final ImageView imageView, String path, @DrawableRes int loadingResId, @DrawableRes int failResId, int width, int height, final DisplayDelegate delegate, boolean loadGif) {
        final String finalPath = getPath(path);
        Activity activity = getActivity(imageView);

        if (HGSystemConfig.IS_PHOTO_PICKER_NEED_GIF && loadGif && finalPath.toLowerCase(Locale.getDefault()).endsWith(".gif")) {
            Glide.with(activity).asGif().load(finalPath).apply(
                    new RequestOptions()
